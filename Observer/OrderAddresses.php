...
class OrderAddresses implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        if (!empty($shippingMethod)) {
            $shippingMethodCarrierCode = explode('_', $shippingMethod)[0];
        }else {
            $shippingMethodCarrierCode = NULL;
        }
        ...
        if ($shippingMethodCarrierCode === $carrierCode && $isEnabled == true) {
            /* --- Shipping & Tax Calculation - START --- */
            ...
            /* --- Shipping & Tax Calculation - END --- */

            /* --- Update RPS On Order - START --- */
            // Store Info - START 
                ...
                if ($productType == \Magento\ConfigurableProduct\Model\Product\Type\Configurable::TYPE_CODE) {
                    ...
                    if ($children) {
                        foreach ($children as $childItem) {
                            ...
                            // Skip virtual and downloadable child products
                            if ($childType == \Magento\Catalog\Model\Product\Type::TYPE_VIRTUAL ||
                                $childType == \Magento\Downloadable\Model\Product\Type::TYPE_DOWNLOADABLE) {
                                continue;
                            }
            ...
            foreach ($order->getAllVisibleItems() as $item) {
                ...
                // Skip virtual or downloadable products
                ...
                // Handle simple products (only if no parent item)
                ...
                elseif ($productType == \Magento\ConfigurableProduct\Model\Product\Type\Configurable::TYPE_CODE) {
                    ...
                    if ($children) {
                        foreach ($children as $childItem) {
                            ...
                            // Skip virtual or downloadable children
                            if ($childType == \Magento\Catalog\Model\Product\Type::TYPE_VIRTUAL ||
                                $childType == \Magento\Downloadable\Model\Product\Type::TYPE_DOWNLOADABLE) {
                                continue;
                            }
                ...
                elseif ($productType == \Magento\Bundle\Model\Product\Type::TYPE_CODE) {
                    ...
                    if ($children) {
                        foreach ($children as $childItem) {
                            ...
                            $hasNonVirtualChild = true;
                        }
                    }

                    // Add bundle product only if it has valid children
                    if ($hasNonVirtualChild) {
                        $data['orderitems'][] = $orderItem;
                    }
                }
            }
            // Send the API request only if the store and shipping countries differ
            if ($countryCode != $shippingAddress->getCountryId()) {
            ...
            /* --- Order Notes - START --- */
            $noPackages        = $shippingAddress->getData('no_packages');
            $packingRuleName   = $shippingAddress->getData('packing_rule_name');
            $packingDimensions = $shippingAddress->getData('packing_dimensions');
            $addressType       = $shippingAddress->getData('address_type_custom');
    ...
    /**
     * Convert CSV string to trimmed array
     *
     * @param string $value
     * @return array
    */
